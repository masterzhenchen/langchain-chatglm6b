...
device = 'cpu'
model_path = r'F:\pythonProject1\bigmodelchatglm6-B\prompt_tasks\PET\checkpoints\model_best'
tokenizer = AutoTokenizer.from_pretrained(model_path)
model = AutoModelForMaskedLM.from_pretrained(model_path)
model.to(device).eval()
max_label_len = 2
verbalizer = Verbalizer(
    verbalizer_file=r'data/verbalizer.txt', tokenizer=tokenizer, max_label_len=max_label_len
)
...
def inference(contents: List[str]):
    ...
    with torch.no_grad():
        ...
        tokenized_output = convert_example(
            examples, tokenizer, hard_template=hard_template, max_seq_len=128, max_label_len=max_label_len,
            train_model=False, return_tensor=True
        )
